...
module Messages
  ...
  def get_messages(conversation_type, messages_amount)
    ...
    Rails.logger.debug "[get_messages] conversation_id=#{params[:conversation_id].inspect}, messages_to_display_offset=#{params[:messages_to_display_offset].inspect}"
    ...
    Rails.logger.debug "[get_messages] @conversation.present?=#{@conversation.present?}, @messages_count=#{@messages.try(:size)}"
